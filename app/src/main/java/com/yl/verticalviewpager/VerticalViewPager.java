        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (Math.abs(velocityY) > criticalVelocityY) {
                    ...
                    if ((getScrollY() >= childHeight * currentPageIndex + childHeight / 10 && !shouZhiXiangXiaHuaDong)) {
                        //手指向上滑动并且，滚动距离过了屏幕一半的距离
                        mScroller.startScroll(0, getScrollY(), 0, childHeight * (currentPageIndex + 1) - getScrollY());
                        currentPageIndex++;

                    } else if ((getScrollY() < childHeight * currentPageIndex + childHeight / 10 && !shouZhiXiangXiaHuaDong)) {
                        //手指向上滑动并且，滚动距离没有过屏幕一半的距离
                        mScroller.startScroll(0, getScrollY(), 0, childHeight * currentPageIndex - getScrollY());

                    } else if ((getScrollY() <= childHeight * (currentPageIndex - 1) + childHeight / 10 * 9 && shouZhiXiangXiaHuaDong)) {
